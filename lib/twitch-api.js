...
exports.getTeamLiveChannels = function(team, callback) {
	...
	// 	channelsNameOnly = globalVars.channels[team];

	// 	exports.getStreamStatus(channelsNameOnly, function(error, errorType, response) {
	// 					if (!error) {
	// 						for (var i = 0; i < response.streams.length;) {
	...
	// 								channels[i].viewers = response.streams[i].viewers;
	// 								i++;
	// 							}
								
	// 							else {channels.splice(i, 1); response.streams.splice(i, 1);}
	// 						}
		...
		function(callback) {
			requestTeamChannelList(team, page, function(error, errorType, response, pageTotal) {
				if (!error) {
					...
					for (var i = 0; i < response.length; i++) {
						channelsNameOnly.push(response[i].username);
					}
