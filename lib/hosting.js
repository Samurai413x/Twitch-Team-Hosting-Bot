...
exports.chooseChannel = function(team, channel, manualHostLength, preferredGame) {
	...
	async.waterfall([
		function(callback) {
			if (!globalVars.isUsingManualChannelList[team] && globalVars.teamLastCheck[team] < moment().unix() - globalVars.teamRecheckLength) {
		...
		function(callback) {
			...
			if (!chosenChannel) {
				getOnlineChannels(team, function(onlineChannels) {
					if (globalVars.simpleMode) {
						onlineChannels = globalVars.channels[team];
						loggingFuncs.logMessage(team, 'Simple Mode overrode online channels to: ' + onlineChannels);
					}
					if (team) {
						if (!globalVars.simpleMode) {
						...
						if (onlineChannels.length > 0) {
							...
							// More than 1...
								...
								loggingFuncs.logMessage(team, 'channelsToChoose: ' + channelsToChoose + '.');
								...
								// Chooses a channel.
								// TODO: AJK - restore old version and wrap this in simple mode if
								chosenChannel = channelsToChoose[looper % channelsToChoose.length];
								looper++;
								loggingFuncs.logMessage(team, 'chosenChannel: ' + chosenChannel + '.');
							}
						}
					...
					// If there was an error getting the online team channels, fake as if we have no channel to host.
					else {
						chosenChannel = null;
						callback(null);
					}
		...
		loggingFuncs.logMessage(team, 'chosenChannel 2 : ' + chosenChannel + '.');	
		if (chosenChannel) {
			...
			getOfflineChannels(team, otherTeamChannels, function(offlineChannels) {
				loggingFuncs.logMessage(team, 'Offline Channels available to serve as hosts ' + offlineChannels);
...
function getOfflineChannels(team, channels, callback) {
	if (globalVars.simpleMode) {
		callback(["NAME OF THE MAIN STREAM GOES HERE"]);
	} else {
